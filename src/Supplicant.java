...
public class Supplicant {
				...
				if (status == -1) {
					...
					if (reconnect_enable.equals("true") && connectCnt > 0) {
						if (++retryCnt <= 10) { // 10次重连机会
							...
							System.out.println("Try to reconnect failed " + (retryCnt - 1) + " times, had to give up.");
							break;
						}
					} else {
						break;
					...
					System.out.println("\nKeep connecting failed. Try to reconnect...");
				} else {
					break;
				}
			}
	...
	private void loadData(String[] args) throws Exception {
		...
		try {
			...
			if (!checkNetwork()) {
				throw new Exception(
						"The network card is not available, " + "please check whether the network card is disabled?");
			}
			initUdpSocket();
	...
	private void initUdpSocket() {
		if (udpSocket == null) {
			try {
				...
				System.out.println(
						"Failed to bind socket to port:3848.\n" + "Please check whether the port is being used?");
				System.exit(0);
			}
		}
	}
	...
	private int breathe(byte[] session) throws InterruptedException {
		...
		while (true) {
			...
			try {
				...
				timeoutCnt++;
				if (timeoutCnt < 4) {
					System.out.println("Failed to connect the " + SERVICE_TYPE + ": " + e.getMessage() + ". "
							+ "Try to reconnect(" + timeoutCnt + ")...");
					continue;
				} else {
					System.out.println(
							"Try to reconnect " + (timeoutCnt - 1) + " times have been failed, had to give up.");
					return -2; // 连接超时、出错，下线
				}
			}
			index += 3;
	...
	private byte[] connect(byte[] packet) {
		...
		try {
			...
			if (checkMD5(recvMd5, getMD5Bytes(recvPacket))) {
				...
				System.out.println("Failed to connect the " + SERVICE_TYPE + ". Try to reconnect...");
